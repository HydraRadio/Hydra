...
def construct_rhs_mpi(
    ...
    for k in range(Nants):
        my_y[k,:] = pspec_sqrt * np.tensordot(FFc, 
                                              yy[k, :, :].flatten(), 
                                              axes=((1,), (0,)))
...
def apply_operator_mpi(comm, x, inv_noise_var, pspec_sqrt, A_real, A_imag, model_vis, Fbasis):
    """
    ...
    for k in range(y.shape[0]):
        yy[k, :] = pspec_sqrt * np.tensordot(FFc, 
                                             y[k, :, :].flatten(), 
                                             axes=((1,), (0,)))
