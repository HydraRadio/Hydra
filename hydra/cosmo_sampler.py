...
def precompute_mpi(comm,
    ...
    v_re = (proj.real * np.sqrt(inv_noise_var_chunk[...,np.newaxis])).reshape((-1, Npix))
    v_im = (proj.imag * np.sqrt(inv_noise_var_chunk[...,np.newaxis])).reshape(((-1, Npix)))
    ...
    for j in range(freq_chunk.size):
        ...
        my_linear_op[i,:,:] = v_re[:,j,:,:].T @ v_re[:,j,:,:] \
                            + v_im[:,j,:,:].T @ v_im[:,j,:,:]
    del v_re, v_im
