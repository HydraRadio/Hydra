...
if __name__ == "__main__":
    ...
    ant_pos = build_hex_array(
        hex_spec=(3, 4), d=14.6
    )  # builds array with (3,4,3) ants = 10 total
    ants = list(ant_pos.keys())
    lmax = 20
    nside = 128
    beam_diameter = 14.0
    ...
    lsts_hours = np.linspace(0.0, 8.0, 10)  # in hours for easy setting
    lsts = np.deg2rad(
        (lsts_hours / 24) * 360
    )  # in radian, used by HYDRA (and this code)
    delta_time = 60  # s
    delta_freq = 1e06  # (M)Hz
    latitude = (
        31.7215 * np.pi / 180
    )  # HERA loc in decimal numbers ## There's some sign error in the code, so this missing sign is a quick fix
    solver = cg

    vis_response, autos, ell, m = vis_proj_operator_no_rot(
        freqs=freqs,
        lsts=lsts,
        beams=beams,
        ant_pos=ant_pos,
    ...
    inv_prior_var = 1.0 / prior_cov
    a_0 = (
        np.random.randn(x_true.size) * np.sqrt(prior_cov) + x_true
    )  # gaussian centered on alms with S variance
    ...
    lhs_linear_op = LinearOperator(matvec=lhs_operator, shape=lhs_shape)

    # Get the Wiener Filter solution for initial guess
    wf_soln, wf_convergence_info = solver(
        A=lhs_linear_op,
        b=rhs_wf,
        # tol = 1e-07,
        maxiter=15000,
    ...
    def samples(key):
        ...
        omega_1 = (
            np.random.randn(model_true.size) + 1.0j * np.random.randn(model_true.size)
        ) / np.sqrt(2)

        # Construct the right hand side
        rhs = construct_rhs_no_rot(
            data_vec, inv_noise_var, inv_prior_var, omega_0, omega_1, a_0, vis_response
        )

        # Run and time solver
        time_start_solver = time.time()
        x_soln, convergence_info = solver(
            A=lhs_linear_op,
            b=rhs,
            # tol = 1e-07,
            maxiter=15000,
        ...
        # Save output
        np.savez(
            path + "results_" + f"{data_seed}_" + f"{random_seed}",
            omega_0=omega_0,
            omega_1=omega_1,
            key=key,
            x_soln=x_soln,
    ...
    np.savez(
        path + "precomputed_data_" + f"{data_seed}_" + f"{jobid}",
