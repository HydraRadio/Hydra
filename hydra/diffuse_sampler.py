...
def spectral_index_segments_basic(
    maps, freqs, boundaries, region_npix=None, fill_masked=True, avg_fn=np.mean
):
    """
    ...
    beta = np.log(maps[1] / maps[0]) / np.log(freqs[1] / freqs[0])
...
def calc_proj_operator_per_region(
    ...
    theta_gal, phi_gal = hp.pix2ang(
        nside=nside, ipix=np.arange(m[0].size), lonlat=False
    )
    theta_eq, phi_eq = hp.Rotator(coord="ge", deg=False)(theta_gal, phi_gal)

    # Empty array of per-point source visibilities
    vis_regions = np.zeros(
        (Nvis, freqs.size, times.size, Nregions), dtype=np.complex128
    )
